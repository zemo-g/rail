...
3. **Comprehend's 0/5 is a semantic ceiling, not a sampling ceiling.** Tasks like "complete this factorial so it prints 120" require the model to generate both a function body AND a complete `main = ...` block de novo (these prompts, unlike the other five bands, don't end with `main = `). A last-ditch N=300 sweep (1,500 compiler-graded samples, 18 hours) produced **zero compile-passes** — cleanly falsifying the sampling hypothesis. The model's distribution has visible mass on "function body" but essentially zero mass on "function body + main block." This is what diagnostic-corpus training (Spur-Fix, see `DIAGNOSTIC_CORPUS.md`) is designed to address. See `PHASE_5H_COMPREHEND_CEILING.md` for full forensics.
