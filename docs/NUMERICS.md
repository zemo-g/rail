...
- **Two representations, one boundary (2026-09-11).** A float the compiler
  ...
  object the generic runtime reads, and a generic value read as a float goes
  through `_rail_fval` (tagged int converted, box loaded, anything else taken
  as raw bits). Before this, a float taken out of a tuple reached `_rail_add`
  as raw bits and was dereferenced. Cost: one 16-byte allocation per float
  stored into a container. `float_arr` is unaffected (always raw).
