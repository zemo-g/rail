...
1. **Type the compiler.** The AST becomes an ADT with typed fields, and
   ...
   (`rail types tools/types.rail`: zero errors), and so does the ARM64 code generator. The other
   passes move over one at a time (the checks, the optimizer, the AD and auth synthesis, the
   x86, wasm, Cortex-M and RISC-V backends); two adapters convert between the typed tree and the
   older list form until the last one does.
