...
1. **Type the compiler.** The AST becomes an ADT with typed fields, and
   ...
   (`rail types tools/types.rail`: zero errors), and so do the ARM64 and x86 code generators.
   The other passes move over one at a time (the checks, the optimizer, the AD and auth
   synthesis, the wasm, Cortex-M and RISC-V backends); two adapters convert between the typed
   tree and the older list form until the last one does.
