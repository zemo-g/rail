...
- **Compiler source**: `tools/compile.rail` (~9,300 lines) and `tools/types.rail` (type inference, ~1,150)
- **Seed binary**: `rail_native` (984K ARM64): checked into repo, self-compile produces byte-identical output (fixed point)
...
- **Tests**: `./rail_native test`: 244/244 on master (2026-09-23). Count fluctuates only when concurrent sessions collide on `/tmp/rail_out`; rerun with `--out-prefix` to confirm.
...
```bash
./rail_native test                    # run the suite (244/244)
...
python3 tools/fuzz/gc_stress.py      # every suite test and tree program under RAIL_GC_STRESS
                                      #   (collect every Nth allocation, poison what is freed)
...
- **Polymorphic show**: `show` works on ints, floats, strings, lists (including nested), nil, tuples `(1, 2)` and constructed values `Some (Pair 1 2)`, and prints `true`/`false` where the compiler knows the value is a bool. Closures are not supported.
...
- **Float representation comes from the types** (`tools/types.rail`, `docs/TYPES.md`): a parameter or result whose type is float travels as raw bits; one that untyped data (`dyn`) can reach stays generic. `show(user_func(1.0))` still won't pick the float printer; use `show_float` explicitly.
...
Read `docs/COMPILER_BOOTSTRAP.md` before editing `tools/compile.rail`: bootstrap cycles, the diagnostic pattern, the data-section and ASCII-only traps. Always: `RAIL_ARENA_MB=6000 ./rail_native self`, then `./rail_native test` (244/244), then self again and `cmp` for the fixed point.
