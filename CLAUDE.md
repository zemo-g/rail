...
- **Tests**: `./rail_native test`: 233/233 on master (2026-09-23). Count fluctuates only when concurrent sessions collide on `/tmp/rail_out`; rerun with `--out-prefix` to confirm.
...
```bash
./rail_native test                    # run the suite (233/233)
...
Read `docs/COMPILER_BOOTSTRAP.md` before editing `tools/compile.rail`: bootstrap cycles, the diagnostic pattern, the data-section and ASCII-only traps. Always: `RAIL_ARENA_MB=6000 ./rail_native self`, then `./rail_native test` (233/233), then self again and `cmp` for the fixed point.
