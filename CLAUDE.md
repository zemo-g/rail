...
- **Cross-function float return inference**: Works via `__fret_` markers in the arity map; v3.6.1's `returns_float` rewrite added let-bound float tracking through V (variable) AST nodes. Cross-function *parameter* inference still requires explicit annotations, and `show(user_func(1.0))` still won't auto-detect float return at the call site — use `show_float` when the return type isn't obvious from local context.
