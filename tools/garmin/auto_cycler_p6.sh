#!/usr/bin/env bash
# auto_cycler_p6.sh - Pass-5 variant: queue is "subdir <space> src_fit_path"
# pairs, one per line.

set -u
queue=$HOME/garmin_recon/fuzz/auto_cycler_queue_p6.txt
...
resume_dir=$(ls -dt $HOME/garmin_recon/fuzz/cycle_*_p6-*/ 2>/dev/null | head -10 |
             while read d; do
               d=${d%/}
