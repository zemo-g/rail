#!/usr/bin/env bash
# auto_cycler_p5.sh - Pass-5 variant: queue is "subdir <space> src_fit_path"
# pairs, one per line.

set -u
queue=$HOME/garmin_recon/fuzz/auto_cycler_queue_p5.txt
...
resume_dir=$(ls -dt $HOME/garmin_recon/fuzz/cycle_*_p5-*/ 2>/dev/null | head -10 |
             while read d; do
               d=${d%/}
...
while true; do
  ...
  case "$state:$cur_state" in
    READY:MOUNTED|UNKNOWN:MOUNTED)
      ...
      result=$(/Users/user/projects/rail/tools/garmin/fuzz_phase_a_dir_p5.sh "$target_subdir" "$src_fit" 2>&1)
