...
check_fleet_reach() {
  ...
  for entry in "${FLEET_NODES[@]}"; do
    ...
    local body=$(curl -s --max-time 3 -H "X-Fleet-Token: $token" "http://$ip:9101/health" 2>/dev/null)
    if echo "$body" | grep -q '"alive": true'; then
      event fleet_reach ok "$name:$ip"
    else
      missed+=("$name")
      log WARN "fleet unreachable: $name ($ip)"
      event fleet_reach unreachable "$name:$ip"
    fi
  done
  if [ "${#missed[@]}" -ge 2 ]; then
