#!/usr/bin/env python3
# LM10 (rung 20): the FOREIGN RE-VERIFIER that closes the loop for the char-LM whose transformer
...
#
# lm10_attested_train.rail produced an Ed25519-signed, hash-chained ledger of a bit-exact
...
#
# What changed from LM8 (and ONLY this changed): the single attention head becomes H=LM10_NH heads,
...
#
# Usage: python3 tools/bitexact/lm10_foreign_check.py [/tmp/lm10_chain.txt]
...
S = 16777216
LM10_NH = 2          # rung-20 heads; dh = d // LM10_NH (d=8 -> dh=4). Mirrors Rail lm10_nh.
LN2 = 11629080
...
# ===================== rung-20 MULTI-HEAD wrappers (mirror lm10 Rail mh_attn_*) =====================
...
def mh_attn_fwd(Wq, Wk, Wv, xs, isd):
    d = len(xs[0]); dh = d // LM10_NH
    outs = None
    for h in range(LM10_NH):
...
def mh_attn_bwd(Wq, Wk, Wv, xs, isd, dOut):
    T = len(xs); d = len(xs[0]); dh = d // LM10_NH
    dWq = []; dWk = []; dWv = []
    dX = [[0]*d for _ in range(T)]
    for h in range(LM10_NH):
        Wqh = Wq[h*dh:(h+1)*dh]; Wkh = Wk[h*dh:(h+1)*dh]; Wvh = Wv[h*dh:(h+1)*dh]
...
# ===== LM10 N=2 trajectory re-derivation (trains W1,W2,E + block0[7] + block1[7] = 17 matrices) =====
...
def main():
    ledger = sys.argv[1] if len(sys.argv) > 1 else "/tmp/lm10_chain.txt"
    ...
    if not lines or not lines[0].startswith("# LM10v1"):
        print("LM10-CHECK FAIL: missing/malformed ledger header")
        sys.exit(1)
    hdr = lines[0].split()
    ...
    print(f"LM10-CHECK corpus pin: SHA-256 reproduced = {py_corpus_sha == corpus_sha}")
    print(f"LM10-CHECK data layer: vocab={len(vocab)} (match {len(vocab)==vsize}) tokens={len(ids)} "
          f"pairs={len(pairs)} indim={cwin*d} ff={ff} isd={ISD} nheads={LM10_NH} nblocks=2 "
          f"(2 STACKED pre-norm transformer blocks: each RMSNorm+{LM10_NH}-head-RoPE-causal-attn"
          f"+residual, then RMSNorm+FFN+residual; cross-entropy)")
    print(f"LM10-CHECK genesis reproduced = {py_genesis == genesis_hex}; "
          f"pubkey re-derived from seed = {py_pub.hex() == pubkey_hex}")
    print(f"LM10-CHECK trajectory: {len(recs)} checkpoints re-derived from data+config+seed "
          f"(every step re-run, {epochs} epochs); field mismatches = {field_mism}")
    print(f"LM10-CHECK signatures: {sigs_ok}/{len(records)} verify under pubkey (RFC 8032)")
    print(f"LM10-CHECK chain head match = {head == ledger_head}; "
          f"datasetLoss0={ds0} datasetLossK={dsK} descent={descent}")
    print(f"LM10-CHECK falsification: corrupt-checkpoint-detected={tamper_detected} "
          f"flipped-sig-rejected={sig_reject}")

    allok = ok and descent and tamper_detected and sig_reject
    if allok:
        print(f"LM10-CHECK PASS: independent foreign re-verifier reproduced every signed checkpoint "
              f"bit-for-bit from data+config+seed ({len(records)} records) for a char-LM that is "
              f"{LM10_NH}-head attention over 2 STACKED pre-norm transformer blocks (each "
