#!/usr/bin/env python3
# LM4 (rung 15): the FOREIGN RE-VERIFIER that closes the loop for the char-LM trained under a
# SOFTMAX + CROSS-ENTROPY objective (rung 14 / LM3 was MSE).
#
# lm4_attested_train.rail produced an Ed25519-signed, hash-chained ledger of a bit-exact
# char-level next-token Rail LM training run. Given only the ledger header (pubkey, genesis,
# epoch count, corpus hash, dims, vocab size, context window, Adam constants) plus the public
# seed/genesis STRINGS and the pinned corpus file, this INDEPENDENT party reconstructs the
# ENTIRE run from scratch in pure-Python big-integers and proves -- bit-for-bit -- that every
# signed CHECKPOINT reproduces. The verifier re-runs EVERY step (full epochs).
#
# What changed from LM3 (and ONLY this changed):
#   * Loss: MSE  ->  cross-entropy. loss = m + ln(sum exp(z_i - m)) - z[tgt], the exact
#     logsumexp form, with the fixed-point natural log fxln() and the fixed-point exp fxexp().
#   * Upstream gradient: dz2 = z2 - onehot(tgt)  ->  dz2 = softmax(z2) - onehot(tgt)
#     = l_cegrad(z2, tgt). softmax/exp/ln are the SAME bit-exact transcendentals the Rail uses
#     (fxexp imported unchanged from bx4; fxln validated bit-for-bit vs the golden LN table).
#   Everything downstream of dz2 -- dW2, dh1, dz1, dW1, dx, the embedding scatter, the Adam
#   step, the gradient clip (cap +/-2^30), w_hex = SHA256(W1;;W2;;E), the chain, the RFC-8032
#   signatures -- is BYTE-IDENTICAL to LM3 and IMPORTED UNCHANGED from the sibling witnesses.
#
# The cross-entropy core (softmax + cross_entropy + ce_grad + fxln) was independently validated
# bit-for-bit against the proven Rail kernel's golden output (sm_golden.out, 36/36 lines) before
# being wired in here -- see /tmp/lm4_ce_validate.py.
#
...
#
# Usage: python3 tools/bitexact/lm4_foreign_check.py [/tmp/lm4_chain.txt]
...
def clipg(g):
    ...
    # +/-2^30 so g*g <= 2^60 < 2^62 in the Adam v-update (no int63 overflow). Under cross-
    # entropy dz2 in [-S,S] is already bounded, but the deeper embedding gradient dx=W1^T.dz1
    # can still exceed the cap; W1/W2 grads pass through.
    if g > CAP:
        return CAP
    if g < -CAP:
        return -CAP
...
# Range-reduce m into [1,2) by doubling/halving (counting k), then evaluate the atanh series.
# All division is truncate-toward-zero (td) -- matches ARM sdiv / the Rail's `/`.
def _ln_up(m, k, s):
    while m < s:
        m += m; k -= 1
    return k
...
# ===================== LM4 data layer (deterministic; mirrors the Rail) =====================
...
def grads4(w1, w2, x, tgt, hidden, indim):
    # MLP backward under CROSS-ENTROPY + dx = W1^T . dz1 -- mirrors lm4_grads.
    # ONLY dz2 differs from LM3: dz2 = softmax(z2) - onehot(tgt) instead of z2 - onehot(tgt).
    z1 = matvec(w1, x)
    h1 = geluv(z1)
    z2 = matvec(w2, h1)
    dz2 = l_cegrad(z2, tgt)
...
def dsloss(pairs, w1, w2, emb):
    # whole-corpus cross-entropy: sum of per-pair l_celoss(forward(ctx), tgt)
    return sum(l_celoss(forward(w1, w2, ctx_vec(emb, ctx)), tgt) for ctx, tgt in pairs)


# ============= LM4 trajectory re-derivation (exact-integer, trains W1, W2 AND E) =============
def rederive(d, hidden, epochs, vsize, cwin, lr, eps, b1, b2, genesis_hex, pairs):
    ...
    for e in range(epochs):
        for ctx, tgt in pairs:
            ...
            dW1, dW2, dx = grads4(w1, w2, x, tgt, hidden, indim)
...
def main():
    ledger = sys.argv[1] if len(sys.argv) > 1 else "/tmp/lm4_chain.txt"
    SEED_STR = "lm4.local.ephemeral.dev.seed.v1"
    GENESIS_STR = "LM4.LOCAL.BEACON.GENESIS.dev"

    with open(ledger) as fh:
        lines = [ln.rstrip("\n") for ln in fh if ln.strip()]
    if not lines or not lines[0].startswith("# LM4v1"):
        print("LM4-CHECK FAIL: missing/malformed ledger header")
        sys.exit(1)
    hdr = lines[0].split()
    # # LM4v1 <pubkey> <genesis> <epochs> <corpus_sha> d=.. hidden=.. vsize=.. cwin=.. beta1=.. beta2=.. lr=.. eps=..
    ...
    with open(os.path.join(repo_root(), "tools/bitexact/lm4_corpus.txt"), "rb") as fh:
        raw = fh.read()
    ...
    ds0 = dsloss(pairs, thetas(initcells(0, hidden, cwin * d, 0)), thetas(initcells(1, vsize, hidden, 0)), emb0)
    ...
    print(f"LM4-CHECK corpus pin: SHA-256 reproduced = {py_corpus_sha == corpus_sha}")
    print(f"LM4-CHECK data layer: vocab={len(vocab)} (match {len(vocab)==vsize}) tokens={len(ids)} "
          f"pairs={len(pairs)} indim={cwin*d} (TRAINED embeddings, cross-entropy loss)")
    print(f"LM4-CHECK genesis reproduced = {py_genesis == genesis_hex}; "
          f"pubkey re-derived from seed = {py_pub.hex() == pubkey_hex}")
    print(f"LM4-CHECK trajectory: {len(recs)} checkpoints re-derived from data+config+seed "
          f"(every step re-run, {epochs} epochs); field mismatches = {field_mism}")
    print(f"LM4-CHECK signatures: {sigs_ok}/{len(records)} verify under pubkey (RFC 8032)")
    print(f"LM4-CHECK chain head match = {head == ledger_head}; "
          f"datasetLoss0={ds0} datasetLossK={dsK} descent={descent}")
    print(f"LM4-CHECK falsification: corrupt-checkpoint-detected={tamper_detected} "
          f"flipped-sig-rejected={sig_reject}")

    allok = ok and descent and tamper_detected and sig_reject
    if allok:
        print(f"LM4-CHECK PASS: independent foreign re-verifier reproduced every signed checkpoint "
              f"bit-for-bit from data+config+seed ({len(records)} records) for a char-LM trained under "
              f"SOFTMAX + CROSS-ENTROPY (w_hex binds W1;;W2;;E); a forged checkpoint or signature is rejected.")
        sys.exit(0)
    print("LM4-CHECK FAIL")
    sys.exit(1)
